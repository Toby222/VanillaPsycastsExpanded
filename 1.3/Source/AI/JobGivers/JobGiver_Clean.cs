    ...
    public class JobGiver_Clean : ThinkNode_JobGiver
    {
        ...
        protected override Job TryGiveJob(Pawn pawn)
        {
            ...
            Predicate<Thing> predicate = (Thing x) =>
                x.def.category == ThingCategory.Filth && HasJobOnThing(pawn, x);
            ...
            {
                job.targetQueueA.SortBy(
                    (LocalTargetInfo targ) => targ.Cell.DistanceToSquared(pawn.Position)
                );
            }
            return job;
            bool ShouldClean(IntVec3 c)
            {
                ...
                if (region != null && !region.links.NullOrEmpty())
                {
                    for (int k = 0; k < region.links.Count; k++)
                    {
                        ...
                        {
                            if (
                                regionLink.regions[l] != null
                                && regionLink.regions[l] != region
                                && regionLink.regions[l].valid
                                && regionLink.regions[l].Room == room
                            )
                            {
                                return true;
                            }
