    ...
    public class Vortex : ThingWithComps
    {
        ...
        public override void Tick()
        {
            ...
            if (this.sustainer == null)
                this.sustainer = VPE_DefOf.VPE_Vortex_Sustainer.TrySpawnSustainer(this);
            ...
            if (Find.TickManager.TicksGame - this.startTick > DURATION)
                this.Destroy();
            if (this.IsHashIntervalTick(30))
                foreach (
                    Pawn pawn in GenRadial
                        .RadialDistinctThingsAround(this.Position, this.Map, RADIUS, true)
                        .OfType<Pawn>()
                )
                    if (pawn.RaceProps.IsMechanoid)
                        pawn.stances.stunner.StunFor(30, this, false);
                    else if (
                        pawn.health.hediffSet.GetFirstHediffOfDef(VPE_DefOf.VPE_Vortex) is null
                    )
                    {
                        Hediff_Vortexed hediff = (Hediff_Vortexed)
                            HediffMaker.MakeHediff(VPE_DefOf.VPE_Vortex, pawn);
                        hediff.Vortex = this;
        ...
        private Vector3 RandomLocation() =>
            this.DrawPos
            + new Vector3(Wrap(Mathf.Abs(Rand.Gaussian(0f, RADIUS)), RADIUS), 0f, 0f).RotatedBy(
                Rand.Range(0f, 360f)
            );
                    ...
                    capMods = new List<PawnCapacityModifier>
                    {
                        new()
                        {
                            ...
                            setMax = Mathf.Lerp(
                                0.25f,
                                0.8f,
                                this.pawn.Position.DistanceTo(this.Vortex.Position) / Vortex.RADIUS
                            ),
                        },
                        new()
                        {
