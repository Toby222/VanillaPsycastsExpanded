...
public class Ability_Thunderbolt : Ability
{
    public override void Cast(params GlobalTargetInfo[] targets)
    {
        ...
        foreach (GlobalTargetInfo target in targets)
        {
            foreach (Thing thing in target.Cell.GetThingList(target.Map).ListFullCopy())
                thing.TakeDamage(
                    new DamageInfo(
                        ...
                        25,
                        -1f,
                        this.pawn.DrawPos.AngleToFlat(thing.DrawPos),
                        this.pawn
                    )
                );
            ...
            this.pawn.Map.weatherManager.eventHandler.AddEvent(
                new WeatherEvent_LightningStrike(this.pawn.Map, target.Cell)
            );
        }
    }
}
