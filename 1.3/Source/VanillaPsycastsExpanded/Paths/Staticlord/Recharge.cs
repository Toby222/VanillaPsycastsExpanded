...
public class HediffComp_Recharge : HediffComp_Draw
{
    ...
    public override void CompPostTick(ref float severityAdjustment)
    {
        ...
        if (this.sustainer == null)
            this.sustainer = VPE_DefOf.VPE_Recharge_Sustainer.TrySpawnSustainer(this.Pawn);
        this.sustainer.Maintain();
        if ((Find.TickManager.TicksGame - this.startTick) % 60 == 0)
            this.battery.TryGetComp<CompPowerBattery>().AddEnergy(100f);
    }
    ...
    public override void DrawAt(Vector3 drawPos)
    {
        ...
        Matrix4x4 matrix = Matrix4x4.TRS(
            drawPos + (b - drawPos) / 2,
            Quaternion.LookRotation(b - drawPos),
            s
        );
...
public class Ability_Recharge : Ability
{
    ...
    public override bool ValidateTarget(LocalTargetInfo target, bool showMessages = true)
    {
        ...
        if (showMessages)
            Messages.Message(
                "VPE.MustTargetBattery".Translate(),
