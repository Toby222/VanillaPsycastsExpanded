...
public class Ability_GuardianSkipBarrier : Ability, IChannelledPsycast
{
    ...
    {
        Hediff hediff = this.pawn.health.hediffSet.GetFirstHediffOfDef(
            VPE_DefOf.VPE_GuardianSkipBarrier
        );
        if (hediff != null)
            return new Command_Action
            {
                ...
                order =
                    10f
                    + (this.def.requiredHediff?.hediffDef?.index ?? 0)
                    + (this.def.requiredHediff?.minimumLevel ?? 0),
            };
