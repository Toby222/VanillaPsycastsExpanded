...
public class Hediff_Essence : HediffWithComps
{
    ...
    public override bool ShouldRemove =>
        this.EssenceOf == null
        || (this.EssenceOf.Dead && this.EssenceOf.Corpse is not { Spawned: true });

    public override void Tick()
    {
        base.Tick();
        if (
            this.EssenceOf is { Dead: true, Corpse: { Spawned: true } }
            && this.pawn.CurJobDef != VPE_DefOf.VPE_EssenceTransfer
        )
        {
...
public class Ability_EssenceTransfer : Ability
{
    ...
    public override void Cast(params GlobalTargetInfo[] targets)
    {
        ...
        if (this.curTarget is { Dead: false, Discarded: false, Destroyed: false })
            foreach (
                Hediff_Essence hediffEssence in this
                    .curTarget.health.hediffSet.hediffs.OfType<Hediff_Essence>()
                    .ToList()
            )
                this.curTarget.health.RemoveHediff(hediffEssence);
        else
            this.curTarget = null;
        Hediff_Essence hediff = (Hediff_Essence)
            HediffMaker.MakeHediff(VPE_DefOf.VPE_Essence, animal);
...
public class JobDriver_EssenceTransfer : JobDriver
{
    ...
    protected override IEnumerable<Toil> MakeNewToils()
    {
        ...
        toil.AddPreInitAction(
            delegate
            {
                this.restStartTick = Find.TickManager.TicksGame;
            }
        );
        toil.AddPreTickAction(
            delegate
            {
                if (Find.TickManager.TicksGame >= this.restStartTick + GenDate.TicksPerHour * 6)
