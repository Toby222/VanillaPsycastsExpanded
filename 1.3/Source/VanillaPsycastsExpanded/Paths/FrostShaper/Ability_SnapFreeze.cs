    ...
    public class Ability_SnapFreeze : Ability
    {
        ...
        public override bool ValidateTarget(LocalTargetInfo target, bool showMessages = true)
        {
            AbilityExtension_Hediff hediffExtension =
                this.def.GetModExtension<AbilityExtension_Hediff>();
            if (
                hediffExtension.targetOnlyEnemies
                && target.Thing != null
                && !target.Thing.HostileTo(this.pawn)
            )
            {
                if (showMessages)
                    Messages.Message(
                        "VFEA.TargetMustBeHostile".Translate(),
                        target.Thing,
        ...
        public static void ApplyHediff(Ability ability, LocalTargetInfo targetInfo)
        {
            ...
            if (targetInfo.Pawn == null || !(hediffExtension?.applyAuto ?? false))
                return;
            BodyPartRecord bodyPart =
                hediffExtension.bodyPartToApply != null
                    ? ability
                        .pawn.health.hediffSet.GetNotMissingParts()
                        .FirstOrDefault(x => x.def == hediffExtension.bodyPartToApply)
                    : null;
            Hediff localHediff = HediffMaker.MakeHediff(
                hediffExtension.hediff,
                targetInfo.Pawn,
                bodyPart
            );
            ...
            if (hediffExtension.durationMultiplier != null)
                duration = (int)(
                    duration * targetInfo.Pawn.GetStatValue(hediffExtension.durationMultiplier)
                );
