    ...
    public class AbilityExtension_WordOfLove : AbilityExtension_AbilityMod
    {
        public override void Cast(GlobalTargetInfo[] targets, Ability ability)
        {
            ...
            Hediff firstHediffOfDef = pawn.health.hediffSet.GetFirstHediffOfDef(
                HediffDefOf.PsychicLove
            );
            if (firstHediffOfDef != null)
            {
                pawn.health.RemoveHediff(firstHediffOfDef);
            }
            Hediff_PsychicLove hediff_PsychicLove = (Hediff_PsychicLove)
            ...
            HediffComp_Disappears hediffComp_Disappears =
                hediff_PsychicLove.TryGetComp<HediffComp_Disappears>();
            if (hediffComp_Disappears != null)
            {
                hediffComp_Disappears.ticksToDisappear = (int)(
                    ability.GetDurationForPawn() * pawn.GetStatValue(StatDefOf.PsychicSensitivity)
                );
            }
        ...
        )
        {
            ...
            if (targets.Any())
            {
                ...
                )
                {
                    ...
                    Gender gender2 = (
                        pawn.story.traits.HasTrait(TraitDefOf.Gay) ? gender : gender.Opposite()
                    );
                    if (pawn2.gender != gender2)
                    {
                        ...
                        {
                            Messages.Message(
                                "AbilityCantApplyWrongAttractionGender".Translate(pawn, pawn2),
                                pawn,
            ...
            foreach (var target in targets)
            {
                ...
                if (pawn != null)
                {
                    if (pawn.story.traits.HasTrait(TraitDefOf.Asexual))
                    {
                        ...
                        {
                            Messages.Message(
                                "AbilityCantApplyOnAsexual".Translate(ability.def.label),
                                pawn,
                                MessageTypeDefOf.RejectInput,
                                historical: false
                            );
