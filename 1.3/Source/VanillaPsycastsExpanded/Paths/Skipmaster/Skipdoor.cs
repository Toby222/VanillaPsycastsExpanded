...
public class Skipdoor : ThingWithComps, IMinHeatGiver
{
    ...
    private static readonly Texture2D DestroyIcon = ContentFinder<Texture2D>.Get(
        "Effects/Skipmaster/Skipgate/SkipdoorDestroy"
    );
    private static readonly Texture2D RenameIcon = ContentFinder<Texture2D>.Get(
        "Effects/Skipmaster/Skipgate/SkipdoorRename"
    );
    private static readonly Material MainMat = MaterialPool.MatFrom(
        "Effects/Skipmaster/Skipgate/Skipgate",
    ...
    private static Pair<Texture2D, Texture2D> GetBackgroundTextures(ModContentPack content)
    {
        Texture2D bg = null,
            mask = null;
        foreach (
            (string path, FileInfo file) in ModContentPack.GetAllFilesForMod(
                content,
                SKIPGATE_TEXPATH,
        ...
        )
        {
            ...
            string name = key.Replace(SKIPGATE_TEXPATH + "/", "")
                .Replace(Path.GetExtension(key), "");
            switch (name)
            {
    ...
    private static void CacheBackground(Texture2D bg, Texture2D mask)
    {
        ...
        for (int y = 0; y < bg.height; y++)
        {
            ...
            Color result =
                maskColor.IndistinguishableFromFast(Color.black) || maskColor.r <= MASK_THRESHOLD
                    ? Color.red
                    : Color.black;
        ...
        {
            name = "Skipdoor_Static_BackgroundMask",
            color = Color.clear,
        };
    ...
    public override void Tick()
    {
        ...
        if (this.distortAmount >= 3f)
            this.distortAmount = 1.5f;
        ...
        if (this.IsHashIntervalTick(30) && this.HitPoints < this.MaxHitPoints)
            this.HitPoints += 1;
    }
    ...
    public override void DrawAt(Vector3 drawLoc, bool flip = false)
    {
        Graphics.DrawMesh(
            ...
            Matrix4x4.TRS(
                drawLoc,
                Quaternion.AngleAxis(this.rotation, Vector3.up),
                Vector3.one * 1.5f
            ),
            MainMat,
            0
        ...
        if (this.backgroundMat != null)
            Graphics.DrawMesh(
                MeshPool.plane10,
                Matrix4x4.TRS(
                    drawLoc - Altitudes.AltIncVect / 2,
                    Quaternion.identity,
                    Vector3.one * 1.5f
                ),
                this.backgroundMat,
        ...
        Graphics.DrawMesh(
            MeshPool.plane10,
            Matrix4x4.TRS(
                drawLoc.Yto0() + Vector3.up * AltitudeLayer.MoteOverhead.AltitudeFor(),
                Quaternion.identity,
    ...
    public override IEnumerable<FloatMenuOption> GetFloatMenuOptions(Pawn selPawn)
    {
        ...
        foreach (
            Skipdoor skipdoor in WorldComponent_SkipdoorManager.Instance.Skipdoors.Except(this)
        )
            yield return new FloatMenuOption(
                "VPE.TeleportTo".Translate(skipdoor.Name),
                () =>
                {
