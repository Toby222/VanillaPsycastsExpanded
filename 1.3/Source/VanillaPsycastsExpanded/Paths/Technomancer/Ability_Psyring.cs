        ...
        public override void Notify_Unequipped(Pawn pawn)
        {
            ...
            if (!this.alreadyHad)
                pawn.GetComp<CompAbilities>()
                    .LearnedAbilities.RemoveAll(ab => ab.def == this.ability);
            this.alreadyHad = false;
        }

        [HarmonyPatch(typeof(FloatMenuMakerMap), "AddHumanlikeOrders")]
        [HarmonyPostfix]
        public static void EquipConditions(
            Vector3 clickPos,
            Pawn pawn,
            ...
            if (pawn.apparel != null)
            {
                ...
                for (int i = 0; i < thingList.Count; i++)
                    if (thingList[i] is Psyring psyring)
                    {
                        ...
                        FloatMenuOption floatMenuOption = opts.FirstOrDefault(x =>
                        ...
                        if (floatMenuOption != null)
                        {
                            if (pawn.Psycasts() == null)
                            {
                                ...
                                opts.Add(
                                    new FloatMenuOption(
                                        "CannotWear".Translate(psyring.LabelShort, psyring)
                                            + " ("
    ...
    public class Dialog_CreatePsyring : Window
    {
        ...
        public override void DoWindowContents(Rect inRect)
        {
            ...
            foreach (AbilityDef ability in this.possibleAbilities)
            {
                ...
                if (
                    Widgets.ButtonText(
                        rect.TakeBottomPart(20f),
                        "VPE.CreatePsyringButton".Translate()
                    )
                )
                {
                    this.Create(ability);
