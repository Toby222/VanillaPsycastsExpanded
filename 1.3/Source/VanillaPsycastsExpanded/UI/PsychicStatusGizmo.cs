namespace VanillaPsycastsExpanded.UI
{
    ...
    public class PsychicStatusGizmo : Gizmo
    {
        ...
        private static readonly Texture2D EntropyBarTex = SolidColorMaterials.NewSolidColorTexture(
            new Color(0.46f, 0.34f, 0.35f)
        );

        private static readonly Texture2D EntropyBarTexAdd =
            SolidColorMaterials.NewSolidColorTexture(new Color(0.78f, 0.72f, 0.66f));

        private static readonly Texture2D OverLimitBarTex =
            SolidColorMaterials.NewSolidColorTexture(new Color(0.75f, 0.2f, 0.15f));
        ...
        private static readonly Texture2D PsyfocusBarTexReduce =
            SolidColorMaterials.NewSolidColorTexture(new Color(0.65f, 0.83f, 0.83f));
        ...
        private static readonly Texture2D EmptyBarTex = SolidColorMaterials.NewSolidColorTexture(
            new Color(0.03f, 0.035f, 0.05f)
        );

        private static readonly Texture2D PsyfocusTargetTex =
        ...
        )
        {
            ...
            AbilityExtension_Psycast psycast = (
            ...
            if (psycast != null)
            {
                ...
                if (entropyGain > 1.401298E-45f)
                {
                    ...
                    float num3 = this.tracker.EntropyToRelativeValue(
                        this.tracker.EntropyValue + entropyGain
                    );
                    ...
                    rect6.width = Widgets.AdjustCoordToUIScalingFloor(
            ...
            if (this.tracker.EntropyValue > this.tracker.MaxEntropy)
                foreach (
                    KeyValuePair<
                        ...
                        float
                    > keyValuePair in Pawn_PsychicEntropyTracker.EntropyThresholds
                )
                    if (keyValuePair.Value > 1f && keyValuePair.Value < 2f)
                        DrawThreshold(rect5, keyValuePair.Value - 1f, entropyRelativeValue);

            string label =
                this.tracker.EntropyValue.ToString("F0")
                + " / "
                + this.tracker.MaxEntropy.ToString("F0");
            Text.Font = GameFont.Small;
            ...
            Widgets.FillableBar(
                rect8,
                Mathf.Min(this.tracker.CurrentPsyfocus, 1f),
            ...
            if (psycast != null)
            {
                ...
                if (usedPsyfocus > 1.401298E-45f)
                {
                    ...
                    rect9.width = Widgets.AdjustCoordToUIScalingCeil(
                        (this.tracker.CurrentPsyfocus - num5) * width2
                    );
            ...
            for (int i = 1; i < Pawn_PsychicEntropyTracker.PsyfocusBandPercentages.Count - 1; i++)
                DrawThreshold(
                    ...
                    this.tracker.CurrentPsyfocus
                );
            float num6 = Mathf.Clamp(
                Mathf.Round(
                    (Event.current.mousePosition.x - (rect8.x + 3f)) / (rect8.width - 8f) * 16f
                ) / 16f,
                0f,
                1f
            );
            ...
            if (
                ...
                && current.button == 0
                && this.draggingPsyfocusBar
                && flag
            )
            {
                if (Math.Abs(num6 - this.selectedPsyfocusTarget) > 1.401298E-45f)
                    SoundDefOf.DragSlider.PlayOneShotOnCamera();
                this.selectedPsyfocusTarget = num6;
                current.Use();
            }
            ...
            DrawPsyfocusTarget(
                rect8,
                this.draggingPsyfocusBar ? this.selectedPsyfocusTarget : this.tracker.TargetPsyfocus
            );
            GUI.color = Color.white;
            Rect rect10 = rect2;
            rect10.y += 38f;
            ...
            TooltipHandler.TipRegion(
                rect10,
                () => this.tracker.PsyfocusTipString(this.selectedPsyfocusTarget),
            ...
            if (this.tracker.Pawn.IsColonistPlayerControlled)
            {
                ...
                if (
                    Widgets.ButtonImage(
                        limitButton,
                        this.tracker.limitEntropyAmount ? this.LimitedTex : this.UnlimitedTex
                    )
                )
                {
                ...
                TooltipHandler.TipRegion(
                    ...
                    () =>
                        "PawnTooltipPsychicEntropyPainFocus".Translate(
                            this.tracker.Pawn.health.hediffSet.PainTotal.ToStringPercent("F0"),
                            recoveryBonus
                        ),
