...
public class ITab_Pawn_Psycasts : ITab
{
        ...
        foreach (ThingDef def in DefDatabase<ThingDef>.AllDefs)
            if (def.race is { Humanlike: true })
            {
                ...
                def.inspectorTabsResolved?.Add(
                    InspectTabManager.GetSharedInstance(typeof(ITab_Pawn_Psycasts))
                );
            }
    }
    ...
    public ITab_Pawn_Psycasts()
    {
        ...
        this.pathsByTab = DefDatabase<PsycasterPathDef>
            .AllDefs.GroupBy(def => def.tab)
            .ToDictionary(group => group.Key, group => group.ToList());
        this.foci = DefDatabase<MeditationFocusDef>
            .AllDefs.OrderByDescending(def => def.modContentPack.IsOfficialMod)
            .ThenByDescending(def => def.label)
            .ToList();
        this.tabs = this
            .pathsByTab.Select(kv => new TabRecord(
                kv.Key,
                () => this.curTab = kv.Key,
                () => this.curTab == kv.Key
            ))
    ...
    public override bool IsVisible =>
        ...
        && pawn.health.hediffSet.HasHediff(VPE_DefOf.VPE_PsycastAbilityImplant)
    ...
    protected override void FillTab()
    {
        ...
        Widgets.Label(
            bar,
            $"{this.hediff.experience.ToStringByStyle(ToStringStyle.FloatOne)} / {xpForNext}"
        );
        ...
        if (
            listing.ButtonTextLabeled(
                "VPE.PsycasterStats".Translate()
                    + (this.smallMode ? $" ({"VPE.Hover".Translate()})" : ""),
                "VPE.Upgrade".Translate()
            )
        )
        {
        ...
        if (this.smallMode)
        {
            ...
            if (Mouse.IsOver(rect))
            {
                ...
                Find.WindowStack.ImmediateWindow(
                    145 * 62346,
                    new Rect(GenUI.GetMouseAttachedWindowPos(size.x, size.y), size),
                    WindowLayer.Super,
                    delegate
                    {
                        ...
                        inner.StatDisplay(
                            TexPsycasts.IconNeuralHeatLimit,
                            StatDefOf.PsychicEntropyMax,
                            this.pawn
                        );
                        inner.StatDisplay(
                            TexPsycasts.IconNeuralHeatRegenRate,
                            StatDefOf.PsychicEntropyRecoveryRate,
                            this.pawn
                        );
                        inner.StatDisplay(
                            TexPsycasts.IconPsychicSensitivity,
                            StatDefOf.PsychicSensitivity,
                            this.pawn
                        );
                        inner.StatDisplay(
                            TexPsycasts.IconPsyfocusGain,
                            StatDefOf.MeditationFocusGain,
                            this.pawn
                        );
                        inner.StatDisplay(
                            TexPsycasts.IconPsyfocusCost,
                            VPE_DefOf.VPE_PsyfocusCostFactor,
                            this.pawn
                        );
                        inner.End();
                    }
        ...
        {
            if (
                listing.ButtonTextLabeled("VPE.PsysetCustomize".Translate(), "VPE.Edit".Translate())
            )
        ...
        if (!this.smallMode)
        {
            ...
            Widgets.BeginScrollView(
                psysets.ContractedBy(3f, 6f),
                ref this.psysetsScrollPos,
                viewRect
            );
        ...
        if (Prefs.DevMode)
            listing.CheckboxLabeled("VPE.DevMode".Translate(), ref this.devMode);
        this.psysetSectionHeight =
            pawnAndStats.height - prePsysetHeight - (listing.CurHeight - postPsysetHeight);
        listing.End();
        ...
        {
            TabDrawer.DrawTabs(
                new Rect(paths.x, paths.y + 40f, paths.width, paths.height),
                this.tabs
            );
            paths.yMin += 40f;
            Widgets.DrawMenuSection(paths);
    ...
    public void DoPsysets(Rect inRect)
    {
        ...
        if (
            Widgets.ButtonText(
                listing.GetRect(70f).LeftHalf().ContractedBy(5f),
                "VPE.CreatePsyset".Translate()
            )
        )
        {
    ...
    private void DoPaths(Rect inRect)
    {
        ...
        foreach (
            PsycasterPathDef def in this.pathsByTab[this.curTab]
                .OrderByDescending(path => this.hediff.unlockedPaths.Contains(path))
                .ThenBy(path => path.order)
                .ThenBy(path => path.label)
        )
        {
            Texture2D texture = this.useAltBackgrounds
                ? def.backgroundImage
                : def.altBackgroundImage;
            ...
            {
                if (def.HasAbilities)
                    PsycastsUIUtility.DoPathAbilities(rect, def, this.abilityPos, this.DoAbility);
            }
            else
            {
                Widgets.DrawRectFast(
                    rect,
                    new Color(0f, 0f, 0f, this.useAltBackgrounds ? 0.7f : 0.55f)
                );
                if (this.hediff.points >= 1 || this.devMode)
                {
                    ...
                    else
                    {
                        ...
                        centerRect.width = Mathf.Max(
                            centerRect.width,
                            Text.CalcSize(label).x + 10f
                        );
    ...
    private void DoAbility(Rect inRect, AbilityDef ability)
    {
        ...
        if (!this.compAbilities.HasAbility(ability))
        {
            if (
                this.devMode
                || (
                    ability.Psycast().PrereqsCompleted(this.compAbilities)
                    && this.hediff.points >= 1
                )
            )
                unlockable = true;
            else
        ...
        if (unlockable)
            QuickSearchWidget.DrawStrongHighlight(inRect.ExpandedBy(12f));
