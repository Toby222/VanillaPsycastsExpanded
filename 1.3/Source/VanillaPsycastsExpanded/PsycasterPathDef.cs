...
public class PsycasterPathDef : Def
{
    ...
    [Unsaved]
    public Texture2D backgroundImageLocked;

    [Unsaved]
    public Texture2D altBackgroundImageLocked;
    ...
    public virtual bool CanPawnUnlock(Pawn pawn) =>
        this.PawnHasCorrectBackstory(pawn) && this.PawnHasMeme(pawn);

    private bool PawnHasMeme(Pawn pawn) =>
        this.requiredMeme == null || (pawn.Ideo?.memes.Contains(this.requiredMeme) ?? false);

    private bool PawnHasCorrectBackstory(Pawn pawn)
    {
        if (this.requiredBackstoriesAny.NullOrEmpty())
            return true;
        foreach (BackstoryCategoryAndSlot requirement in this.requiredBackstoriesAny)
        {
            List<string> list6 =
                requirement.slot == BackstorySlot.Adulthood
                    ? pawn.story.adulthood?.spawnCategories
                    : pawn.story.childhood?.spawnCategories;
            if (list6 != null && list6.Contains(requirement.categoryName))
                return true;
        }
    ...
    public override void PostLoad()
    {
        ...
        LongEventHandler.ExecuteWhenFinished(
            delegate
            {
                if (!this.background.NullOrEmpty())
                    this.backgroundImage = ContentFinder<Texture2D>.Get(this.background);
                if (!this.altBackground.NullOrEmpty())
                    this.altBackgroundImage = ContentFinder<Texture2D>.Get(this.altBackground);
                ...
                if (this.width > 0 && this.height > 0)
                {
                    ...
                    for (int i = 0; i < colors.Length; i++)
                        colors[i] = this.backgroundColor;

                    tex.SetPixels(colors);
                    tex.Apply();

                    if (this.backgroundImage == null)
                        this.backgroundImage = tex;
                    if (this.altBackgroundImage == null)
                        this.altBackgroundImage = tex;
                }
    ...
    public override void ResolveReferences()
    {
        ...
        foreach (AbilityDef abilityDef in DefDatabase<AbilityDef>.AllDefsListForReading)
        {
            AbilityExtension_Psycast psycast =
                abilityDef.GetModExtension<AbilityExtension_Psycast>();
            if (psycast is not null && psycast.path == this)
        ...
        foreach (
            IGrouping<int, AbilityDef> abilityDefs in this.abilities.GroupBy(ab =>
                ab.Psycast().level
            )
        )
            this.abilityLevelsInOrder[abilityDefs.Key - 1] = abilityDefs
                .OrderBy(ab => ab.Psycast().order)
                .SelectMany(ab =>
                    ab.Psycast().spaceAfter
