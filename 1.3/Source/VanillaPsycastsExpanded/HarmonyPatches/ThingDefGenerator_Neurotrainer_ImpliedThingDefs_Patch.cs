...
{
    public static Func<ThingDef> BaseNeurotrainer = AccessTools
        .Method(typeof(ThingDefGenerator_Neurotrainer), "BaseNeurotrainer")
        .CreateDelegate<Func<ThingDef>>();
    ...
    public static IEnumerable<ThingDef> ImpliedThingDefs()
    {
        foreach (AbilityDef abilityDef in DefDatabase<AbilityDef>.AllDefs)
            if (abilityDef.Psycast() is { } psycastExt)
            {
                ...
                thingDef.descriptionHyperlinks = new List<DefHyperlink> { new(abilityDef) };
...
public class CompPsytrainer : CompUseEffect_GiveAbility
{
    ...
    {
        if (
            this.Props.ability?.Psycast()?.path is { } path
