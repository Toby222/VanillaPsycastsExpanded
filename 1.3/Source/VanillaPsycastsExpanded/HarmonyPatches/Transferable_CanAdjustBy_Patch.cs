    ...
    public static class Transferable_CanAdjustBy_Patch
    {
        ...
        public static void Postfix(Transferable __instance)
        {
            if (
                ...
                && Find.WindowStack.IsOpen<Dialog_Trade>()
                && TradeSession.trader != null
                && __instance.ThingDef.IsEltexOrHasEltexMaterial()
            ...
            )
            {
                ...
                {
                    Messages.Message(
                        "VPE.GiftingEltexWarning".Translate(),
                ...
                {
                    Messages.Message(
                        "VPE.SellingEltexWarning".Translate(),
                        MessageTypeDefOf.CautionInput
                    );
                }
            }
        ...
        public static bool IsEltexOrHasEltexMaterial(this ThingDef def)
        {
            if (def != null)
            {
                if (def == VPE_DefOf.VPE_Eltex)
                {
                    ...
                    def.costList != null
                    && def.costList.Any(x => x.thingDef == VPE_DefOf.VPE_Eltex)
                )
                {
                    return true;
                }
