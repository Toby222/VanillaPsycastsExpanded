...
public class PawnGen_Patch
{
    ...
    public static void Postfix(Pawn __result)
    {
        ...
        if (psycastExtension != null)
        {
            ...
            if (psycastExtension.implantDef != null)
            {
                ...
                Hediff_PsycastAbilities implant = (
                    __result.health.hediffSet.GetFirstHediffOfDef(psycastExtension.implantDef)
                ...
                foreach (PathUnlockData unlockedPath in psycastExtension.unlockedPaths)
                    if (unlockedPath.path.CanPawnUnlock(__result))
                    {
                        ...
                        for (
                            int level = unlockedPath.unlockedAbilityLevelRange.min;
                            level < unlockedPath.unlockedAbilityLevelRange.max
                                && level < unlockedPath.path.MaxLevel;
                            level++
                        )
                            abilitySelection = abilitySelection.Concat(
                                unlockedPath
                                    .path.abilityLevelsInOrder[level - 1]
                                    .Except(PsycasterPathDef.Blank)
                            );
                        ...
                        while (
                            (
                                abilitySelectionListFiltered = abilitySelectionList
                                    .Where(ab => ab.Psycast().PrereqsCompleted(comp))
                                    .ToList()
                            ).Any()
                            && abilityCount > 0
                        )
        ...
        if (
            Find.Storyteller.def == VPE_DefOf.VPE_Basilicus
            && __result.RaceProps.intelligence >= Intelligence.Humanlike
        )
            if (Rand.Value < PsycastsMod.Settings.baseSpawnChance)
            {
                ...
                Hediff_PsycastAbilities implant =
                    __result.health.hediffSet.GetFirstHediffOfDef(
                    ...
                    ?? HediffMaker.MakeHediff(
                        ...
                        __result,
                        __result
                            .RaceProps.body.GetPartsWithDef(BodyPartDefOf.Brain)
                            .FirstOrFallback()
                    ) as Hediff_PsycastAbilities;
                ...
                PsycasterPathDef path = DefDatabase<PsycasterPathDef>
                ...
                IEnumerable<AbilityDef> abilities = path.abilities.Except(
                    comp.LearnedAbilities.Select(ab => ab.def)
                );
                ...
                do
                {
                    if (
                        abilities
                            .Where(ab =>
                                ab.GetModExtension<AbilityExtension_Psycast>()
