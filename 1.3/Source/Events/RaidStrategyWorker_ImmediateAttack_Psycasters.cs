namespace VanillaPsycastsExpanded
{
    ...
    public class RaidStrategyWorker_ImmediateAttack_Psycasters : RaidStrategyWorker_ImmediateAttack
    {
        ...
        {
            IEnumerable<PawnGroupMaker> enumerable = PawnGenOptionsWithRequiredPawns(
                faction,
                groupKind
            );
            if (!enumerable.Any())
            ...
            {
                foreach (
                    PawnGenOption item2 in item.options.Where(
                        (PawnGenOption op) => MatchesRequiredPawnKind(op.kind)
                    )
                )
                {
                    if (item2.Cost < num)
        ...
        public override bool CanUsePawnGenOption(
            float pointsTotal,
            PawnGenOption opt,
            List<PawnGenOption> chosenOpts,
            Faction faction = null
        )
        {
            if (
                chosenOpts.Count < MinRequiredPawnsForPoints(pointsTotal, faction)
                && !MatchesRequiredPawnKind(opt.kind)
            )
            {
                return false;
            }
        ...
        )
        {
            ...
            return faction.def.pawnGroupMakers.Where(
                (PawnGroupMaker gm) =>
                    gm.kindDef == groupKind
                    && gm.options != null
                    && gm.options.Any((PawnGenOption op) => MatchesRequiredPawnKind(op.kind))
            );
        }
    }
}
