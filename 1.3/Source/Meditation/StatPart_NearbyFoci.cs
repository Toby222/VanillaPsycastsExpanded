...
public class StatPart_NearbyFoci : StatPart
{
    ...
    {
        HashSet<MeditationFocusDef> focusTypes = main.TryGetComp<CompMeditationFocus>()
            .Props.focusTypes.ToHashSet();
        List<(Thing thing, List<MeditationFocusDef> foci, float value)> foci = (
            from thing in GenRadialCached.RadialDistinctThingsAround(
                main.Position,
                main.Map,
                MeditationUtility.FocusObjectSearchRadius,
                true
            )
            ...
            select (thing, comp.Props.focusTypes, value)
        ).ToList();
    ...
    public override string ExplanationPart(StatRequest req)
    {
        ...
        List<string> lines = AllFociNearby(req.Thing, req.Pawn)
            .Select(tuple =>
                tuple.thing.LabelCap
                + ": "
                + StatDefOf.MeditationFocusStrength.Worker.ValueToString(
                    tuple.value,
                    true,
