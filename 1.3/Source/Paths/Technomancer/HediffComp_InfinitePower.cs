...
public class HediffComp_InfinitePower : HediffComp_Draw
{
    ...
    public override void DrawAt(Vector3 drawPos)
    {
        Graphics.DrawMesh(
            MeshPool.plane10,
            Matrix4x4.TRS(
                this.target.DrawPos.Yto0() + Vector3.up * AltitudeLayer.MetaOverlays.AltitudeFor(),
    ...
    public override void CompExposeData()
    {
        ...
        if (Scribe.mode == LoadSaveMode.PostLoadInit)
            this.compPower = this.target.TryGetComp<CompPowerTrader>();
    }
}
...
public class Ability_Power : Ability
{
    public override void Cast(params GlobalTargetInfo[] targets)
    {
        ...
        foreach (GlobalTargetInfo target in targets)
            this.ApplyHediff(this.pawn)?.TryGetComp<HediffComp_InfinitePower>().Begin(target.Thing);
    }
