...
public class Ability_Flashstorm : Ability
{
    ...
    public override void Cast(params GlobalTargetInfo[] targets)
    {
        ...
        {
            Map map = target.Map;
            Thing conditionCauser = GenSpawn.Spawn(
                ThingDefOf.Flashstorm,
                target.Cell,
                this.pawn.Map
            );
            GameCondition_PsychicFlashstorm flashstorm = (GameCondition_PsychicFlashstorm)
                GameConditionMaker.MakeCondition(VPE_DefOf.VPE_PsychicFlashstorm);
            flashstorm.centerLocation = target.Cell.ToIntVec2;
            flashstorm.areaRadiusOverride = new IntRange(
                Mathf.RoundToInt(this.GetRadiusForPawn()),
                Mathf.RoundToInt(this.GetRadiusForPawn())
            );
    ...
    private void ApplyGoodwillImpact(GlobalTargetInfo target, int radius)
    {
        ...
        foreach (
            ...
            if (
                thing is Pawn p
                && thing.Faction != null
                && thing.Faction != this.pawn.Faction
                && !thing.Faction.HostileTo(this.pawn.Faction)
                && !this.affectedFactionCache.Contains(thing.Faction)
...
public class GameCondition_PsychicFlashstorm : GameCondition_Flashstorm
{
    private static readonly AccessTools.FieldRef<
        ...
        int
    > nextLightningTicksRef = AccessTools.FieldRefAccess<GameCondition_Flashstorm, int>(
        "nextLightningTicks"
    );
    ...
    private Vector3 RandomLocation() =>
        ...
        + new Vector3(
            Vortex.Wrap(Mathf.Abs(Rand.Gaussian(0f, this.AreaRadius)), this.AreaRadius),
            0f,
            0f
        ).RotatedBy(Rand.Range(0f, 360f));
    ...
    public override void GameConditionTick()
    {
        ...
        for (int i = 0; i < 2; i++)
            FleckMaker.ThrowSmoke(this.RandomLocation(), this.SingleMap, 4f);
    }
