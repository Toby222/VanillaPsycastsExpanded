...
public class TimeSphere : Thing
{
    ...
    public override void SpawnSetup(Map map, bool respawningAfterLoad)
    {
        ...
        foreach (
            ...
            if (
                thing is Pawn { Faction: { IsPlayer: false } } pawn
                && !pawn.Faction.HostileTo(Faction.OfPlayer)
            )
                pawn.Faction.TryAffectGoodwillWith(
                    Faction.OfPlayer,
                    -75,
                    true,
                    true,
    ...
    public override void Tick()
    {
            ...
            )
            {
                ...
                if (thing is Plant plant)
                {
                    if (plant.Growth < 1f)
                        ...
                        thing.TakeDamage(
                            new DamageInfo(DamageDefOf.Deterioration, 0.01f * thing.MaxHitPoints)
                        );
                    else
        ...
        if (this.sustainer == null)
            this.sustainer = VPE_DefOf.VPE_TimeSphere_Sustainer.TrySpawnSustainer(this);
        else
    ...
    public override void Draw()
    {
        Graphics.DrawMesh(
            MeshPool.plane10,
            Matrix4x4.TRS(
                this.Position.ToVector3ShiftedWithAltitude(AltitudeLayer.MoteOverheadLow),
                Quaternion.AngleAxis(0f, Vector3.up),
                Vector3.one * this.Radius * 1.75f
            ),
            DistortionMat,
            0
        );
