...
{
    private static readonly Material DistortionMat = DistortedMaterialsPool.DistortedMaterial(
        "Things/Mote/Black",
    ...
    public override void GameConditionTick()
    {
            ...
            foreach (Map map in this.AffectedMaps)
            {
                ...
                foreach (
                    ...
                    if (plant.Growth < 1f)
                        ...
                        plant.TakeDamage(
                            new DamageInfo(DamageDefOf.Rotting, 0.01f * plant.MaxHitPoints)
                        );
                    else
                        plant.Age = int.MaxValue;

                foreach (
                    Building building in map
                        .listerBuildings.allBuildingsColonist.Concat(
                    ...
                    if (building.def.useHitPoints)
                        building.TakeDamage(
                            new DamageInfo(DamageDefOf.Deterioration, 0.01f * building.MaxHitPoints)
                        );
            }
        ...
        if (this.sustainer == null)
            this.sustainer = VPE_DefOf.Psycast_Neuroquake_CastLoop.TrySpawnSustainer(this.Pawn);
        else
    ...
    private bool CanEffect(Thing thing) =>
        !thing.Position.InHorDistOf(this.Pawn.Position, this.SafeRadius);

    public override void GameConditionDraw(Map map)
    {
        base.GameConditionDraw(map);
        if (Find.Selector.IsSelected(this.Pawn))
            GenDraw.DrawRadiusRing(this.Pawn.Position, this.SafeRadius, Color.yellow);

        Matrix4x4 matrix = Matrix4x4.TRS(
            this.Pawn.Position.ToVector3ShiftedWithAltitude(AltitudeLayer.MoteOverhead),
            Quaternion.AngleAxis(0f, Vector3.up),
            Vector3.one * this.SafeRadius * 2f
        );
...
public class Ability_TimeQuake : Ability
{
    ...
    {
        base.Cast(targets);
        GameCondition_TimeQuake condition = (GameCondition_TimeQuake)
            GameConditionMaker.MakeCondition(VPE_DefOf.VPE_TimeQuake, this.GetDurationForPawn());
        ...
        skyChanger
            .TryGetComp<CompAffectsSky>()
            .StartFadeInHoldFadeOut(0, this.GetDurationForPawn(), 0);
        ...
        foreach (Faction faction in Find.FactionManager.AllFactionsVisible)
        {
            if (faction.CanChangeGoodwillFor(this.pawn.Faction, -10))
                faction.TryAffectGoodwillWith(
                    this.pawn.Faction,
                    -10,
                    reason: HistoryEventDefOf.UsedHarmfulAbility
                );

            if (
                faction.CanChangeGoodwillFor(this.pawn.Faction, -75)
                && this.pawn.Map.mapPawns.SpawnedPawnsInFaction(faction).Count > 0
            )
                faction.TryAffectGoodwillWith(
                    this.pawn.Faction,
                    -75,
...
public class ThoughtWorker_TimeQuake : ThoughtWorker
{
    protected override ThoughtState CurrentStateInternal(Pawn p) =>
        p.Map.gameConditionManager.ConditionIsActive(VPE_DefOf.VPE_TimeQuake);
}
