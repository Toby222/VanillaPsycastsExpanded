...
public class AbilityExtension_Age : AbilityExtension_AbilityMod
{
    ...
    public override void Cast(GlobalTargetInfo[] targets, Ability ability)
    {
        ...
        if (this.casterYears.HasValue)
            Age(ability.pawn, this.casterYears.Value);
        if (!this.targetYears.HasValue)
            return;
    ...
    {
        if (years < 0 && pawn.ageTracker.AgeBiologicalYears <= 16)
            return;
            ...
            foreach (HediffGiver giver in giverSet.hediffGivers)
                if (giver is HediffGiver_Birthday giverBirthday)
                    if (giverBirthday.ageFractionChanceCurve.Evaluate(lifeFraction) <= 0f)
                    {
                        ...
                        while (
                            (
                                hediff = pawn.health.hediffSet.GetFirstHediffOfDef(
                                    giverBirthday.hediff
                                )
                            ) != null
                        )
        ...
        if (pawn.ageTracker.AgeBiologicalYears < 16)
            pawn.ageTracker.AgeTickMothballed(
                GenDate.TicksPerYear * 16 - (int)pawn.ageTracker.AgeBiologicalTicks
            );

        if (pawn.ageTracker.AgeBiologicalYears > pawn.def.race.lifeExpectancy * 1.1f)
        {
            BodyPartRecord part = pawn.RaceProps.body.AllParts.FirstOrDefault(p =>
                p.def == BodyPartDefOf.Heart
            );
