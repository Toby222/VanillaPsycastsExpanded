    ...
    public class Ability_LuckTransfer : Ability
    {
        public override void Cast(params GlobalTargetInfo[] targets)
        {
            ...
            int duration = Mathf.RoundToInt(
                this.GetDurationForPawn() * source.GetStatValue(StatDefOf.PsychicSensitivity)
            );

            source
                .health.AddHediff(VPE_DefOf.VPE_Lucky)
                .TryGetComp<HediffComp_Disappears>()
                .ticksToDisappear = duration;
            dest
                .health.AddHediff(VPE_DefOf.VPE_UnLucky)
                .TryGetComp<HediffComp_Disappears>()
                .ticksToDisappear = duration;
        }
    }
