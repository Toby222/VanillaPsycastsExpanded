namespace VanillaPsycastsExpanded.Graphics
{
    ...
    public class Graphic_AnimatedMote : Graphic_Animated
    {
        public override void Init(GraphicRequest req)
        {
            ...
            foreach (
                IGrouping<string, Texture2D> item in from s in list
            ...
            )
            {
                ...
                {
                    if (
                        list3[num].name.Contains("_east")
                        || list3[num].name.Contains("_north")
                        || list3[num].name.Contains("_west")
                        || list3[num].name.Contains("_south")
                    )
                    {
                        list3.RemoveAt(num);
                        flag = true;
