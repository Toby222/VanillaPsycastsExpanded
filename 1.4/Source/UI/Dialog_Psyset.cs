    ...
    public class Dialog_Psyset : Window
    {
        ...
        public override void DoWindowContents(Rect inRect)
        {
            ...
            DragAndDropWidget.DropArea(
                group,
                existingRect,
                obj => this.psyset.Abilities.Add((AbilityDef)obj),
                null
            );
            ...
            if (this.curIdx > 0 && Widgets.ButtonText(decreaseRect, "<"))
                this.curIdx--;
            if (this.curIdx < this.paths.Count - 1 && Widgets.ButtonText(increaseRect, ">"))
                this.curIdx++;
            Text.Anchor = TextAnchor.MiddleCenter;
            Widgets.Label(
                pagesRect,
                $"{(this.paths.Count > 0 ? this.curIdx + 1 : 0)} / {this.paths.Count}"
            );
            ...
            if (DragAndDropWidget.CurrentlyDraggedDraggable() is AbilityDef abilityDef)
                PsycastsUIUtility.DrawAbility(
                    new Rect(Event.current.mousePosition, new Vector2(36f, 36f)),
                    abilityDef
                );
