namespace VanillaPsycastsExpanded.UI
{
    ...
    public static class PsycastsUIUtility
    {
        ...
        public static void LabelWithIcon(
        ...
        public static Rect CenterRect(this Rect rect, Vector2 size) =>
            new(rect.center - size / 2f, size);

        public static Texture2D Icon(this MeditationFocusDef def) => meditationIcons[def];

        public static void DrawPathBackground(
            ref Rect rect,
            PsycasterPathDef def,
            bool altTex = false
        )
        {
        ...
        public static void DoPathAbilities(
            Rect inRect,
            PsycasterPathDef path,
            Dictionary<AbilityDef, Vector2> abilityPos,
            Action<Rect, AbilityDef> doAbility
        )
        {
            if (!EnsureInit())
                return;
            foreach (AbilityDef def in path.abilities)
                if (
                    def.Psycast()?.prerequisites is { } prerequisites
                    && abilityPos.ContainsKey(def)
                )
                    foreach (
                        AbilityDef abilityDef in prerequisites.Where(abilityDef =>
                            abilityPos.ContainsKey(abilityDef)
                        )
                    )
                        Widgets.DrawLine(
                            abilityPos[def],
                            abilityPos[abilityDef],
                            CompAbilities.HasAbility(abilityDef) ? Color.white : Color.grey,
                            2f
                        );
            ...
            for (int level = 0; level < path.abilityLevelsInOrder.Length; level++)
            {
                Rect levelRect =
                    new(
                        inRect.x,
                        inRect.y
                            + (path.MaxLevel - 1 - level) * inRect.height / path.MaxLevel
                            + 10f,
                        inRect.width,
                        inRect.height / 5f
                    );
                ...
                for (int pos = 0; pos < abilities.Length; pos++)
                {
                    Rect rect =
                        new(
                            levelRect.x
                                + levelRect.width / 2
                                + abilityTreeXOffsets[abilities.Length - 1][pos],
                            levelRect.y,
                            36f,
                            36f
                        );
                    AbilityDef def = abilities[pos];
                    if (def == PsycasterPathDef.Blank)
                        continue;
