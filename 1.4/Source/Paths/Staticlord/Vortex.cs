        ...
        if (this.IsHashIntervalTick(30))
            foreach (
                var pawn in GenRadial
                    .RadialDistinctThingsAround(Position, Map, RADIUS, true)
                    .OfType<Pawn>()
            )
...
public class Hediff_Vortexed : Hediff
{
                ...
                capMods = new()
                {
                    new()
                    {
                        ...
                        setMax = Mathf.Lerp(
                            0.5f,
                            0.9f,
                            pawn.Position.DistanceTo(Vortex.Position) / Vortex.RADIUS
                        ),
                    },
                    new()
                    {
    ...
    public override bool ShouldRemove =>
        Vortex.Destroyed || pawn.Position.DistanceTo(Vortex.Position) >= Vortex.RADIUS;
