...
public class Ability_Mend : Ability
{
        ...
        foreach (var target in targets)
            if (target.Thing is Pawn p)
            {
                if (p.RaceProps.Humanlike)
                {
                    ...
                    var toHeal = p
                        .equipment.AllEquipmentListForReading.Concat(p.apparel.WornApparel)
                    ...
                    if (i >= maxIterations)
                        Log.Warning(
                            $"[VPE] Too many iterations in Ability_Mend.Cast by {pawn} on {p}"
                        );
                }
                else if (p.RaceProps.IsMechanoid)
                {
                    ...
                    while (amountTotal >= 0 && toHeal.Count > 0 && i++ <= maxIterations)
                        toHeal.RemoveAll(injury =>
                        {
                            var amount = Mathf.Clamp(
                                amountTotal >= 1f ? amountTotal / toHeal.Count : amountTotal,
                                0,
                                injury.Severity
                            );
                            injury.Heal(amount);
                    ...
                    if (toHeal.Count == 0)
                        MechRepairUtility.RepairTick(p);
                }
            }
            else
    ...
    public override float GetPowerForPawn() =>
        (pawn.GetStatValue(StatDefOf.PsychicSensitivity) - 1) * 100f;
    ...
    private static float Mend(Thing t, float amount) =>
        Mend(t, (int)amount) + (amount - (int)amount);
    ...
    public override bool ValidateTarget(LocalTargetInfo target, bool showMessages = true)
    {
        ...
        if (target.Thing is Pawn p)
        {
            if (p.RaceProps.Humanlike)
            {
                ...
                if (
                    !p.equipment.AllEquipmentListForReading.Any(t =>
                        t.def.useHitPoints && t.HitPoints < t.MaxHitPoints
                    )
                    && !p.apparel.WornApparel.Any(t =>
                        t.def.useHitPoints && t.HitPoints < t.MaxHitPoints
                    )
                )
                {
                    if (showMessages)
                        Messages.Message(
                            "VPE.MustHaveDamagedEquipment".Translate(),
                            MessageTypeDefOf.RejectInput,
                            false
                        );
            ...
            if (showMessages)
                Messages.Message("VPE.NoAnimals".Translate(), MessageTypeDefOf.RejectInput, false);
