namespace VanillaPsycastsExpanded
{
    ...
    public class Ability_ConsumeBodies : Ability_TargetCorpse
    {
                    ...
                    foreach (var target in Comp.currentlyCastingTargets)
                    {
                        if (target.HasThing && target.Thing.TryGetComp<CompRottable>() != null)
                        {
                            this.AddEffecterToMaintain(
                                VPE_DefOf.VPE_Liferot.Spawn(target.Thing.Position, this.pawn.Map),
                                target.Thing,
            ...
            toil.AddPreTickAction(
                delegate
                {
                    foreach (var target in Comp.currentlyCastingTargets)
                    {
                        if (
                            target.HasThing
                            && target.Thing.TryGetComp<CompRottable>() != null
                            && target.Thing.IsHashIntervalTick(60)
                        )
                        {
        ...
        public override void Cast(params GlobalTargetInfo[] targets)
        {
            ...
            var consumerHediff =
                this.pawn.health.hediffSet.GetFirstHediffOfDef(VPE_DefOf.VPE_BodiesConsumed)
