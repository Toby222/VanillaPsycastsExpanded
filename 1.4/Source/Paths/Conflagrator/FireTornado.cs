    ...
    public class FireTornado : ThingWithComps
    {
        ...
        private float FadeInOutFactor
        {
            get
            {
                ...
                float b =
                    this.leftFadeOutTicks < 0 ? 1f : Mathf.Min(this.leftFadeOutTicks / 120f, 1f);
                ...
                else
                {
                    ...
                    if (intVec.InBounds(this.Map))
                    {
                        ...
                        if (this.IsHashIntervalTick(4) && !this.CellImmuneToDamage(this.Position))
                        {
                            ...
                            ThrowPuff(
                                new Vector3(this.realPosition.x, 0f, this.realPosition.y)
                                {
        ...
        private void DoFire()
        {
            foreach (
                IntVec3 cell in GenRadial
                    .RadialCellsAround(this.Position, 4.2f, true)
                    .Where(c => c.InBounds(this.Map) && !this.CellImmuneToDamage(c))
                    .InRandomOrder()
                    .Take(Rand.Range(3, 5))
            )
            {
            ...
            foreach (
                Pawn pawn in GenRadial
                    .RadialDistinctThingsAround(this.Position, this.Map, 4.2f, true)
                    .OfType<Pawn>()
            )
            {
        ...
        private void SpawnChemfuel()
        {
            foreach (
                IntVec3 cell in GenRadial
                    ...
                    .Where(c =>
                        c.InBounds(this.Map)
                        && FilthMaker.CanMakeFilth(c, this.Map, ThingDefOf.Filth_Fuel)
        ...
        )
        {
            ...
            Vector2 vector = this.realPosition.Moved(
                num3,
                AdjustedDistanceFromCenter(distanceFromCenter)
            );
            ...
            Matrix4x4 matrix = Matrix4x4.TRS(
                pos,
                Quaternion.Euler(0f, num3, 0f),
                new Vector3(num4, 1f, num4)
            );
            Graphics.DrawMesh(
                MeshPool.plane10,
                matrix,
        ...
        private void CreateSustainer()
        {
            LongEventHandler.ExecuteWhenFinished(
                delegate
                {
                    ...
                    this.sustainer = tornado.TrySpawnSustainer(
