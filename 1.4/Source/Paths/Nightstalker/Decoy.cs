...
public class Decoy : ThingWithComps, IAttackTarget
{
    ...
    public override void SpawnSetup(Map map, bool respawningAfterLoad)
    {
        ...
        foreach (
            IAttackTarget target in this.pawn.Map.attackTargetsCache.TargetsHostileToFaction(
                this.pawn.Faction
            )
        )
            if (
                target.Thing is Pawn { CurJobDef: var jobDef } targetPawn
                && (jobDef == JobDefOf.Wait_Combat || jobDef == JobDefOf.Goto)
            )
    ...
    {
        if (!this.Map.listerThings.AllThings.OfType<Decoy>().Except(this).Any())
            mapsWithDecoys.Remove(this.Map);
        base.DeSpawn(mode);
    }
    ...
    [HarmonyPrefix]
    public static bool BestAttackTarget_Prefix(
    ...
    )
    {
        if (!mapsWithDecoys.Contains(searcher.Thing.MapHeld))
            return true;
        List<IAttackTarget> list1 = searcher.Thing.Map.attackTargetsCache.GetPotentialTargetsFor(
            searcher
        );
    ...
    {
        if (
            pawn.mindState.enemyTarget is { } and not Decoy
            && mapsWithDecoys.Contains(pawn.MapHeld)
        )
