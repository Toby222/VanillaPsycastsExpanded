namespace VanillaPsycastsExpanded.Nightstalker
{
    ...
    public class Hediff_Darkvision : HediffWithComps
    {
        ...
        {
            if (
                def == VPE_DefOf.EnvironmentDark
                && __result == null
                && DarkvisionPawns.Contains(pawn)
            )
                __result = pawn
                    .health.hediffSet.hediffs.OfType<Hediff_Darkvision>()
                    .FirstOrDefault();
        }
        ...
        {
            if (__result && t is Pawn p && DarkvisionPawns.Contains(p))
                __result = false;
        }
        ...
        public override void PostAdd(DamageInfo? dinfo)
        {
            ...
            foreach (
                BodyPartRecord part in this.pawn.RaceProps.body.AllParts.Where(part =>
                    part.def == BodyPartDefOf.Eye
                )
            )
        ...
        public override void PostRemoved()
        {
            ...
            while (
                (
                    hediff = this.pawn.health.hediffSet.GetFirstHediffOfDef(
                        VPE_DefOf.VPE_Darkvision_Display
                    )
                ) != null
            )
