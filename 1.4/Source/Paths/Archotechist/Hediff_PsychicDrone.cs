namespace VanillaPsycastsExpanded
{
    ...
    public class Hediff_PsychicDrone : Hediff_Overlay
    {
        ...
        public override string OverlayPath =>
            "Effects/Archotechist/PsychicDrone/PsychicDroneEnergyField";
        ...
        public override void PostAdd(DamageInfo? dinfo)
        {
            ...
            maintainedMotes.Add(
                SpawnMoteAttached(
                    VPE_DefOf.VPE_PsycastAreaEffectMaintained,
            ...
            if (Find.TickManager.TicksGame % 180 == 0)
            {
                if (
                    GenRadial
                        ...
                        .OfType<Pawn>()
                        .Where(x =>
                            !affectedPawns.Contains(x)
                            && !x.InMentalState
                            && x.HostileTo(pawn)
                            && x.RaceProps.IsFlesh
                        )
                        .TryRandomElement(out var victim)
                )
                {
                    var mentalStateDef = Rand.Bool
                        ? VPE_DefOf.VPE_Wander_Sad
        ...
        {
            MoteAttachedScaled mote =
                MoteMaker.MakeAttachedOverlay(pawn, moteDef, Vector3.zero) as MoteAttachedScaled;
            mote.maxScale = scale;
