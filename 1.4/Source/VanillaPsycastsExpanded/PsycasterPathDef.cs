...
public class PsycasterPathDef : Def
{
    ...
    public virtual bool CanPawnUnlock(Pawn pawn) =>
        PawnHasCorrectBackstory(pawn)
        && PawnHasMeme(pawn)
        && PawnHasGene(pawn)
        && PawnIsMechanitor(pawn)
        && PawnHasCorrectFocus(pawn);

    private bool PawnHasMeme(Pawn pawn) =>
        requiredMeme == null || (pawn.Ideo?.memes.Contains(requiredMeme) ?? false);

    private bool PawnHasGene(Pawn pawn) =>
        requiredGene == null || (pawn.genes?.GetGene(requiredGene)?.Active ?? false);

    private bool PawnIsMechanitor(Pawn pawn) =>
        !requiredMechanitor || (MechanitorUtility.IsMechanitor(pawn));

    private bool PawnHasCorrectFocus(Pawn pawn) =>
        requiredFocus == null || requiredFocus.CanPawnUse(pawn);
    ...
    private bool PawnHasCorrectBackstory(Pawn pawn)
    {
        ...
        foreach (var requirement in requiredBackstoriesAny)
            if (
                pawn.story.GetBackstory(requirement.slot)?.spawnCategories is { } spawnCategories
                && spawnCategories.Contains(requirement.categoryName)
            )
                return true;
    ...
    public override void ResolveReferences()
    {
        ...
        foreach (var abilityDefs in abilities.GroupBy(ab => ab.Psycast().level))
            abilityLevelsInOrder[abilityDefs.Key - 1] = abilityDefs
