...
{
    public static IEnumerable<CodeInstruction> AddLogs(
    ...
    )
    {
        foreach (var instruction in instructions)
        {
            ...
            if (
                methods.FirstOrDefault(instruction.Calls) is
                { Name: var name, ReturnType: var result }
            )
            {
                ...
                yield return new CodeInstruction(
                    OpCodes.Ldstr,
                    $"{original.DeclaringType?.Name ?? "Free"}.{original.Name}"
                );
