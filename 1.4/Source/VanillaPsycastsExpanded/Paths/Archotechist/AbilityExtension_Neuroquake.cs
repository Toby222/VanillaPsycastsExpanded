    ...
    public class AbilityExtension_Neuroquake : AbilityExtension_AbilityMod
    {
        ...
        public override void Cast(GlobalTargetInfo[] targets, Ability ability)
        {
            ...
            foreach (Pawn item in ability.pawn.Map.mapPawns.AllPawnsSpawned)
            {
                if (CanApplyEffects(item) && !item.Fogged())
                {
                    bool flag =
                        !item.Spawned
                        || item.Position.InHorDistOf(
                            ability.pawn.Position,
                            ability.GetAdditionalRadius()
                        )
                        || !item.Position.InHorDistOf(
                            ability.pawn.Position,
            ...
            {
                if (
                    map == ability.pawn.Map
                    || Find.WorldGrid.TraversalDistanceBetween(
                        map.Tile,
                        ability.pawn.Map.Tile,
                        passImpassable: true,
                        worldRangeTiles + 1
                    ) > worldRangeTiles
                )
            ...
            foreach (Pawn item2 in giveMentalStateTo)
            {
                ...
                mentalStateDef = (
                    item2.RaceProps.IsMechanoid
                        ? MentalStateDefOf.BerserkMechanoid
                        : MentalStateDefOf.Berserk
                );
                AbilityExtension_GiveMentalState.TryGiveMentalStateWithDuration(
                    mentalStateDef,
                    item2,
                    ability,
            ...
            {
                foreach (
                    KeyValuePair<Faction, Pair<bool, Pawn>> affectedFaction in affectedFactions
                )
                {
                    ...
                    int goodwillChange = (
                        first ? goodwillImpactForBerserk : goodwillImpactForNeuroquake
                    );
            ...
            coma.TryGetComp<HediffComp_Disappears>().ticksToDisappear = (int)(
                (GenDate.TicksPerDay * 5) / ability.pawn.GetStatValue(StatDefOf.PsychicSensitivity)
            );
        ...
        private void AffectGoodwill(Faction faction, bool gaveMentalBreak, Pawn p = null)
        {
            if (
                ...
                && (p == null || !p.IsSlaveOfColony)
                && (
                    !affectedFactions.TryGetValue(faction, out var value)
                    || (!value.First && gaveMentalBreak)
                )
            )
            {
