namespace VanillaPsycastsExpanded.Wildspeaker
{
    ...
    public abstract class PlantSpawner : TunnelHiveSpawner
    {
        ...
        [HarmonyTranspiler]
        public static IEnumerable<CodeInstruction> TickTranspiler(
        ...
        public override void SpawnSetup(Map map, bool respawningAfterLoad)
        {
            ...
            {
                secondSpawnTickRef(this) =
                    Find.TickManager.TicksGame
                    + this.DurationTicks()
                    + Rand.RangeInclusive(-60, 120);
                Rand.PushState(Find.TickManager.TicksGame);
                this.plantDef = this.ChoosePlant(this.Position, map);
                Rand.PopState();
            }

            if (!this.CheckSpawnLoc(this.Position, map) || this.plantDef == null)
                this.Destroy();
        }
    ...
    public class BrambleSpawner : PlantSpawner
    {
        protected override void SetupPlant(Plant plant, IntVec3 loc, Map map)
        {
            ...
            if (this.TryGetComp<CompDuration>() is { durationTicksLeft: var ticks })
                Current
                    .Game.GetComponent<GameComponent_PsycastsManager>()
                    .removeAfterTicks.Add((plant, Find.TickManager.TicksGame + ticks));
        }
    ...
    public class WildPlantSpawner : PlantSpawner
    {
        protected override ThingDef ChoosePlant(IntVec3 loc, Map map)
        {
            ...
            if (
                DefDatabase<ThingDef>
                    .AllDefs.Where(td => td.plant is { Sowable: true, IsTree: false })
                    .TryRandomElement(out ThingDef plantDef)
            )
        ...
        {
            base.SetupPlant(plant, loc, map);
            plant.Growth = Mathf.Clamp(
                this.TryGetComp<CompAbilitySpawn>().pawn.GetStatValue(StatDefOf.PsychicSensitivity)
                    - 1f,
                0.1f,
                1f
            );
    ...
    public class TreeSpawner : PlantSpawner
    {
        ...
        {
            if (
                map.Biome.AllWildPlants.Where(td => td.plant is { IsTree: true })
                    .TryRandomElement(out ThingDef plantDef)
                || map.Biome.AllWildPlants.TryRandomElement(out plantDef)
            )
                if (
                    plantDef.CanEverPlantAt(loc, map, true)
                    && PlantUtility.AdjacentSowBlocker(plantDef, loc, map) == null
                )
                    return plantDef;
            return null;
        }
