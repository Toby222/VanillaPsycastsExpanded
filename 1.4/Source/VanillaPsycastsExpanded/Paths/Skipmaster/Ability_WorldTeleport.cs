...
public class Ability_WorldTeleport : Ability
{
    public override void DoAction()
    {
        ...
        if (pawn != null)
            Dialog_MessageBox.CreateConfirmation(
                "FarskipConfirmTeleportingLodger".Translate(pawn.Named("PAWN")),
                base.DoAction
            );
        else
            base.DoAction();
    ...
    private IEnumerable<Pawn> PawnsToSkip()
    {
        ...
        else
        {
            ...
            foreach (
                ...
                if (
                    thing is Pawn { Dead: false } pawn2
                    && (
                        pawn2.IsColonist
                        || pawn2.IsPrisonerOfColony
                        || (
                            !homeMap
                            && pawn2.RaceProps.Animal
                            && pawn2.Faction is { IsPlayer: true }
                        )
                    )
                )
                    yield return pawn2;
    ...
    private Pawn AlliedPawnOnMap(Map targetMap)
    {
        return targetMap.mapPawns.AllPawnsSpawned.FirstOrDefault(p =>
            !p.NonHumanlikeOrWildMan()
            && p.IsColonist
            && p.HomeFaction == Faction.OfPlayer
            && !this.PawnsToSkip().Contains(p)
        );
    }

    private bool ShouldEnterMap(GlobalTargetInfo target)
    {
        if (target.WorldObject is Caravan caravan && caravan.Faction == this.pawn.Faction)
            return false;

        return target.WorldObject is MapParent { HasMap: true } mapParent
            && (this.AlliedPawnOnMap(mapParent.Map) != null || mapParent.Map == this.pawn.Map);
    }
    ...
    public override bool CanHitTargetTile(GlobalTargetInfo target)
    {
        ...
        return (caravan == null || caravan != caravan1)
            && (
                this.ShouldEnterMap(target)
                || (caravan1 != null && caravan1.Faction == this.pawn.Faction)
            )
            ...
            delegate
            {
                ...
                foreach (Pawn p in this.PawnsToSkip())
                {
                    ...
                    this.AddEffecterToMaintain(
                        EffecterDefOf.Skip_Entry.Spawn(this.pawn, this.pawn.Map),
                        this.pawn.Position,
                        60
                    );
                }
    ...
    public override void Cast(params GlobalTargetInfo[] targets)
    {
        ...
        if (this.pawn.Spawned)
            SoundDefOf.Psycast_Skip_Pulse.PlayOneShot(
                new TargetInfo(targets[0].Cell, this.pawn.Map)
            );
        if (
            targetMap != null
            && this.AlliedPawnOnMap(targetMap) is { Position: var alliedPawnCell }
        )
        ...
        if (targetCell.IsValid)
        {
            foreach (Pawn pawn3 in list)
            {
                ...
                CellFinder.TryFindRandomSpawnCellForPawnNear(
                    ...
                    cell =>
                        cell != targetCell
                        && cell.GetRoom(targetMap) == targetCell.GetRoom(targetMap)
                );
                ...
                this.AddEffecterToMaintain(
                    EffecterDefOf.Skip_ExitNoDelay.Spawn(pawn3, pawn3.Map),
                    pawn3.Position,
                    60,
                    targetMap
                );
            ...
            targets[0].WorldObject is Caravan caravan2
            && caravan2.Faction == this.pawn.Faction
        )
        {
            if (caravan != null)
            {
