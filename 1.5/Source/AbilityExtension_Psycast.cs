...
public class AbilityExtension_Psycast : AbilityExtension_AbilityMod
{
    ...
    {
        return prerequisites.NullOrEmpty()
            || compAbilities.LearnedAbilities.Any(ab => prerequisites.Contains(ab.def));
    }
    ...
    public float GetPsyfocusUsedByPawn(Pawn pawn) =>
        psyfocusCost * pawn.GetStatValue(VPE_DefOf.VPE_PsyfocusCostFactor);

    public float GetEntropyUsedByPawn(Pawn pawn) =>
        entropyGainStatFactors.Aggregate(
            entropyGain,
            (current, statFactor) =>
    ...
    public override bool IsEnabledForPawn(Ability ability, out string reason)
    {
        ...
        if (hediff != null)
        {
            ...
            if (
                ability.pawn.psychicEntropy.WouldOverflowEntropy(GetEntropyUsedByPawn(ability.pawn))
            )
            {
    ...
    public override void Cast(GlobalTargetInfo[] targets, Ability ability)
    {
        ...
        psycastHediff.UseAbility(
            GetPsyfocusUsedByPawn(ability.pawn),
