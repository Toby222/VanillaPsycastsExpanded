...
public class Ability_Animal : Ability
{
                ...
                if (p.AnimalOrWildMan())
                {
                    ...
                    if (Rand.Chance(GetSuccessChanceOn(p)))
                    {
                        ...
                        p.mindState.mentalStateHandler.TryStartMentalState(
                            ...
                            "AnimalManhunterFromTaming".Translate(),
                            true,
    ...
    private float GetSuccessChanceOn(Pawn target) =>
        pawn.GetStatValue(StatDefOf.PsychicSensitivity) - target.RaceProps.wildness;

    public override void OnGUI(LocalTargetInfo target)
    {
        base.OnGUI(target);
        var validTargets = currentTargets
            .Where(t => t is { IsValid: true, Map: not null })
            .ToList();
        ...
        targets[targets.Length - 1] = target.ToGlobalTargetInfo(
            validTargets?.LastOrDefault().Map ?? pawn.Map
        );
        ...
        foreach (var targetInfo in targets)
            if (targetInfo.Thing is Pawn p)
            {
                if (p.AnimalOrWildMan() && GetSuccessChanceOn(p) > 1.401298E-45f)
                {
                    ...
                    GenMapUI.DrawText(
                        new(drawPos.x, drawPos.z),
                        "VPE.SuccessChance".Translate() + ": " + chance.ToStringPercent(),
                        color
                    );
                }
                else
