namespace VanillaPsycastsExpanded.Technomancer
{
    ...
    public class Ability_ReverseEngineer : Ability
    {
        ...
        private static readonly AccessTools.FieldRef<
            ResearchManager,
            Dictionary<ResearchProjectDef, float>
        > progressRef = AccessTools.FieldRefAccess<
            ResearchManager,
            Dictionary<ResearchProjectDef, float>
        >("progress");
        ...
        public override void Cast(params GlobalTargetInfo[] targets)
        {
            base.Cast(targets);
            foreach (
                ResearchProjectDef project in GetResearchFor(targets[0].Thing)
                    .Where(project => project is { IsFinished: false })
            )
            {
                int techprints = Find.ResearchManager.GetTechprints(project);
                if (techprints < project.TechprintCount)
                    Find.ResearchManager.AddTechprints(
                        project,
                        techprints - project.TechprintCount
                    );
                ...
                DiaNode diaNode =
                    new(
                        "ResearchFinished".Translate(project.LabelCap)
                            + "\n\n"
                            + project.description
                    );
                ...
                if (
                    !project.discoveredLetterTitle.NullOrEmpty()
                    && Find.Storyteller.difficulty.AllowedBy(project.discoveredLetterDisabledWhen)
                )
                    Find.LetterStack.ReceiveLetter(
                        project.discoveredLetterTitle,
                        project.discoveredLetterText,
        ...
        {
            if (researchCache.TryGetValue(t, out List<ResearchProjectDef> projects))
                return projects;
            projects = new List<ResearchProjectDef>();
            if (!t.def.researchPrerequisites.NullOrEmpty())
                projects.AddRange(t.def.researchPrerequisites);
            if (t.def.recipeMaker != null)
            {
                if (t.def.recipeMaker.researchPrerequisite != null)
                    projects.Add(t.def.recipeMaker.researchPrerequisite);
                if (!t.def.recipeMaker.researchPrerequisites.NullOrEmpty())
                    projects.AddRange(t.def.recipeMaker.researchPrerequisites);
            }
            ...
            foreach (RecipeDef recipeDef in DefDatabase<RecipeDef>.AllDefs)
                if (recipeDef.products.Any(prod => prod.thingDef == t.def))
                {
                    if (recipeDef.researchPrerequisite != null)
                        projects.Add(recipeDef.researchPrerequisite);
                    if (!recipeDef.researchPrerequisites.NullOrEmpty())
                        projects.AddRange(recipeDef.researchPrerequisites);
                }
