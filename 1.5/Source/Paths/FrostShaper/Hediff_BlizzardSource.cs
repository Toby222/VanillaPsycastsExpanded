...
public class Hediff_BlizzardSource : Hediff_Overlay
{
    ...
    public override void PostRemoved()
    {
        ...
        coma.TryGetComp<HediffComp_Disappears>().ticksToDisappear = (int)(
            GenDate.TicksPerDay * 5 / pawn.GetStatValue(StatDefOf.PsychicSensitivity)
        );
    ...
    public override void Tick()
    {
        ...
        if (affectedFactions is null)
            affectedFactions = new();
        ...
        foreach (var victim in affectedPawns)
            ...
            {
                var blizzardHediff = victim.health.hediffSet.GetFirstHediffOfDef(
                    VPE_DefOf.VPE_Blizzard
                );
                ...
                if (
                    victim.IsHashIntervalTick(60) && victim.CanReceiveHypothermia(out var hediffDef)
                )
                {
