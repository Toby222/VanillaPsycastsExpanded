...
public class Ability_Wallraise : Ability
{
    public AbilityExtension_Wallraise Props =>
        this.def.GetModExtension<AbilityExtension_Wallraise>();
    ...
    public override void Cast(params GlobalTargetInfo[] targets)
    {
        ...
        {
            Map map = target.Map;
            LocalTargetInfo localTarget = target.HasThing
                ? new LocalTargetInfo(target.Thing)
                : new LocalTargetInfo(target.Cell);
            List<Thing> items = new();
            items.AddRange(
                this.Props.AffectedCells(localTarget, map)
                    .SelectMany(c =>
            ...
            foreach (IntVec3 loc in this.Props.AffectedCells(localTarget, map))
            {
                ...
                FleckMaker.ThrowDustPuffThick(
                    loc.ToVector3Shifted(),
                    map,
                    Rand.Range(1.5f, 3f),
                    CompAbilityEffect_Wallraise.DustColor
                );
            }
            ...
            foreach (Thing item in items)
            {
                ...
                for (int i = 0; i < 9; i++)
                {
                    ...
                    if (
                        c.InBounds(map)
                        && c.Walkable(map)
                        && map.thingGrid.ThingsListAtFast(c).Count <= 0
                    )
                    {
                        cell = c;
                        break;
    ...
    public override void DrawHighlight(LocalTargetInfo target)
    {
        ...
        GenDraw.DrawFieldEdges(
            this.Props.AffectedCells(target, this.pawn.Map).ToList(),
            this.ValidateTarget(target) ? Color.white : Color.red
        );
    }
    ...
    public override bool ValidateTarget(LocalTargetInfo target, bool showMessages = false)
    {
        ...
        if (this.Props.AffectedCells(target, this.pawn.Map).Any(c => !c.Standable(this.pawn.Map)))
        {
            if (showMessages)
                Messages.Message(
                    "AbilityUnwalkable".Translate(this.def.LabelCap),
                    target.ToTargetInfo(this.pawn.Map),
                    MessageTypeDefOf.RejectInput,
                    false
                );
...
public class AbilityExtension_Wallraise : AbilityExtension_AbilityMod
{
    ...
    internal IEnumerable<IntVec3> AffectedCells(LocalTargetInfo target, Map map) =>
        this
            .pattern.Select(intVec => target.Cell + new IntVec3(intVec.x, 0, intVec.z))
            .Where(intVec2 => intVec2.InBounds(map));
}
