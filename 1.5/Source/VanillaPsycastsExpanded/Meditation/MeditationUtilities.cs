...
public static class MeditationUtilities
{
    [DebugAction(
        "Pawns",
        "Check Meditation Focus Strength",
        true,
    ...
    {
        var value = pawn.GetStatValue(StatDefOf.MeditationFocusGain);
        Log.Message(
            $"Value: {value}, Explanation:\n{StatDefOf.MeditationFocusGain.Worker.GetExplanationFull(StatRequest.For(pawn), ToStringNumberSense.Absolute, value)}"
        );

        var focus = MeditationUtility.BestFocusAt(pawn.Position, pawn);
        if (!focus.HasThing)
            return;
    ...
    {
        if (
            focus == VPE_DefOf.Dignified
            && (
                pawn.royalty == null
                || !pawn.royalty.AllTitlesForReading.Any()
                || !pawn.royalty.CanUpdateTitleOfAnyFaction(out _)
            )
        )
        {
