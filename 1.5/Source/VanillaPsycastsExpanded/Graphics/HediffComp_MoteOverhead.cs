...
{
    private static readonly List<Vector3> animalHeadOffsets =
        new() { new(0f, 0f, 0.4f), new(0.4f, 0f, 0.25f), new(0f, 0f, 0.1f), new(-0.4f, 0f, 0.25f) };
    ...
    public override void CompPostTick(ref float severityAdjustment)
    {
        ...
        if (Pawn.Spawned)
        {
            ...
            var rotation =
                pawn.GetPosture() != 0 ? pawn.Drawer.renderer.LayingFacing()
    ...
    {
        base.CompExposeData();
        if (mote != null && mote.def.CanBeSaved() is false)
            return;
