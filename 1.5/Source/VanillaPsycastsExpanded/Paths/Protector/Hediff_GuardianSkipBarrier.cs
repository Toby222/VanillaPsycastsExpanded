...
public class Hediff_GuardianSkipBarrier : Hediff_Overshield
{
    ...
    public override void PostTick()
    {
        ...
        if (this.sustainer == null || this.sustainer.Ended)
            this.sustainer = VPE_DefOf.VPE_GuardianSkipbarrier_Sustainer.TrySpawnSustainer(
                SoundInfo.InMap(this.pawn, MaintenanceType.PerTick)
            );
    ...
    private void AddEntropy()
    {
        if (Find.TickManager.TicksGame % 10 == 0)
            this.pawn.psychicEntropy.TryAddEntropy(1f, overLimit: true);
