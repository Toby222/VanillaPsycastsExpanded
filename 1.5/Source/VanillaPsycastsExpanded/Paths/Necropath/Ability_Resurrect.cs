namespace VanillaPsycastsExpanded
{
    ...
    public class Ability_Resurrect : Ability_TargetCorpse
    {
        public override Gizmo GetGizmo()
        {
            ...
            var fingerParts = pawn
                .health.hediffSet.GetNotMissingParts()
                .Where(x => x.def == VPE_DefOf.Finger);

            if (
                fingerParts.All(finger =>
                    pawn.health.hediffSet.hediffs.Any(hediff =>
                        hediff.def == VPE_DefOf.VPE_Sacrificed && hediff.Part == finger
                    )
                )
            )
            {
        ...
        public override void Cast(params GlobalTargetInfo[] targets)
        {
            ...
            foreach (var target in targets)
            {
                ...
                )
                {
                    ...
                    var soul =
                        SkyfallerMaker.MakeSkyfaller(VPE_DefOf.VPE_SoulFromSky) as SoulFromSky;
                    soul.target = corpse;
                    ...
                    pawn.health.AddHediff(
                        HediffMaker.MakeHediff(VPE_DefOf.VPE_Sacrificed, pawn, finger),
                        finger
                    );
                }
            }
