...
public class HurricaneMaker : Thing
{
    ...
    {
        if (!this.Pawn.psychicEntropy.TryAddEntropy(1f, this) || this.Pawn.Downed)
            this.Destroy();
    }
...
{
    public static readonly Material HurricaneOverlay = MaterialPool.MatFrom(
        "Effects/Staticlord/Hurricane/VPEHurricaneWorldOverlay",
...
public class HediffComp_Hurricane : HediffComp_SeverityPerDay
{
    ...
    {
        if (this.Pawn.Map.weatherManager.CurWeatherPerceived != VPE_DefOf.VPE_Hurricane_Weather)
