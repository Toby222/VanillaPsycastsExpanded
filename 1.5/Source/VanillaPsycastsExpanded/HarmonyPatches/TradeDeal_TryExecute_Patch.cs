...
public static class TradeDeal_TryExecute_Patch
{
    public static void Prefix(List<Tradeable> ___tradeables, out int __state)
    {
        ...
        foreach (Tradeable tradeable in ___tradeables)
            __state +=
                tradeable.ThingDef.GetEltexOrEltexMaterialCount()
    ...
    public static int GetEltexOrEltexMaterialCount(this ThingDef def)
    {
        if (def != null)
        {
            ...
            {
                ThingDefCountClass firstCost = def.costList.FirstOrDefault(x =>
                    x.thingDef == VPE_DefOf.VPE_Eltex
                );
                if (firstCost != null)
                ...
                foreach (RecipeDef recipe in DefDatabase<RecipeDef>.AllDefs)
                    ...
                    {
                        IngredientCount firstCost = recipe.ingredients.FirstOrDefault(x =>
                    ...
                    .goodwillImpacts.Add(
                        new GoodwillImpactDelayed
                        {
                            ...
                            historyEvent = TradeSession.giftMode
                                ? VPE_DefOf.VPE_GiftedEltex
                                : VPE_DefOf.VPE_SoldEltex,
                            impactInTicks =
                                Find.TickManager.TicksGame
                                + (int)(GenDate.TicksPerDay * Rand.Range(7f, 14f)),
                            letterLabel = "VPE.EmpireAngeredTitle".Translate(),
                            letterDesc = "VPE.EmpireAngeredDesc".Translate(
                                TradeSession.giftMode
                                    ? "VPE.Gifting".Translate()
                                    : "VPE.Trading".Translate()
                            ),
