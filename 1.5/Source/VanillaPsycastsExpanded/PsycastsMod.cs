...
public class PsycastsMod : Mod
{
        ...
        : base(content)
    {
        ...
        Harm.Patch(
            AccessTools.Method(
                typeof(ThingDefGenerator_Neurotrainer),
                nameof(ThingDefGenerator_Neurotrainer.ImpliedThingDefs)
            ),
            postfix: new(
                typeof(ThingDefGenerator_Neurotrainer_ImpliedThingDefs_Patch),
                nameof(ThingDefGenerator_Neurotrainer_ImpliedThingDefs_Patch.Postfix)
            )
        );
        Harm.Patch(
            AccessTools.Method(typeof(GenDefDatabase), nameof(GenDefDatabase.GetDef)),
            new(GetType(), nameof(PreGetDef))
        );
        Harm.Patch(
            AccessTools.Method(typeof(GenDefDatabase), nameof(GenDefDatabase.GetDefSilentFail)),
            new(GetType(), nameof(PreGetDef))
        );
        var conversionChain =
            (List<BackCompatibilityConverter>)
                AccessTools.Field(typeof(BackCompatibility), "conversionChain").GetValue(null);
        conversionChain.Add(psytrainerConverter = new());
        conversionChain.Add(new BackCompatibilityConverter_Constructs());
        if (
            ModsConfig.IsActive("GhostRolly.Rim73") || ModsConfig.IsActive("GhostRolly.Rim73_steam")
        )
            Log.Warning(
    ...
    public override void DoSettingsWindowContents(Rect inRect)
    {
        ...
        listing.Label(
            "VPE.PsycasterSpawnBaseChance".Translate() + ": " + Settings.baseSpawnChance * 100 + "%"
        );
        Settings.baseSpawnChance = listing.Slider(Settings.baseSpawnChance, 0, 1f);
        listing.Label(
            "VPE.PsycasterSpawnAdditional".Translate()
                + ": "
                + Settings.additionalAbilityChance * 100
                + "%"
        );
        Settings.additionalAbilityChance = listing.Slider(Settings.additionalAbilityChance, 0, 1f);
        listing.CheckboxLabeled(
            "VPE.AllowShrink".Translate(),
            ref Settings.shrink,
            "VPE.AllowShrink.Desc".Translate()
        );
        listing.CheckboxMultiLabeled(
            "VPE.SmallMode".Translate(),
            ref Settings.smallMode,
            "VPE.SmallMode.Desc".Translate()
        );
        ...
        listing.CheckboxLabeled(
            "VPE.ChangeFocusGain".Translate(),
            ref Settings.changeFocusGain,
            "VPE.ChangeFocusGain.Desc".Translate()
        );
        listing.End();
    }
    ...
    {
        if (__2 && psytrainerConverter.BackCompatibleDefName(__0, __1) is { } newDefName)
            __1 = newDefName;
    }
}
