...
public class BackCompatibilityConverter_Psytrainers : BackCompatibilityConverter
{
    private static readonly Dictionary<string, string> specialCases =
        new() { { "BulletShield", "VPE_Skipshield" }, { "EntropyDump", "VPE_NeuralHeatDump" } };
    ...
    )
    {
        ...
        {
            string abilityDefName = defName.Replace(
                ThingDefGenerator_Neurotrainer.PsytrainerDefPrefix + "_",
                ""
            );
            if (!abilityDefName.StartsWith("VPE_"))
            {
                if (abilityDefName.StartsWith("WordOf"))
                    abilityDefName = abilityDefName.Replace("WordOf", "Wordof");
                if (!specialCases.TryGetValue(abilityDefName, out string newAbilityDefName))
                    newAbilityDefName = "VPE_" + abilityDefName;
                if (DefDatabase<AbilityDef>.GetNamedSilentFail(newAbilityDefName) is not null)
                    return ThingDefGenerator_Neurotrainer.PsytrainerDefPrefix
                        + "_"
                        + newAbilityDefName;

                Log.Warning(
                    $"[VPE] Failed to find psycast for psytrainer called {newAbilityDefName} (old name: {abilityDefName})"
                );
    ...
    public override Type GetBackCompatibleType(
        Type baseType,
